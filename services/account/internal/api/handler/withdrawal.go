...
// @Failure 409 {object} response.ErrorResponse "Insufficient balance"
// @Failure 500 {object} response.ErrorResponse
//
// @Router /account/{id}/withdraw [put]
func (h *AccountHandler) WithDrawal(c *gin.Context) {
	...
	if err := h.AccUC.WithDrawal(ctx, accID, &req); err != nil {
		...
		if errors.Is(err, errmessage.ErrInsufficientAmount) {
		...
		c.JSON(http.StatusInternalServerError, response.ErrorResponse{
			Error: "Failed to withdraw amount: " + err.Error(),
		})
		return
	}
